...
--->
    ...
    function createCalendar(year, month) {
        ...
        for (let i = 0; i < 7; i++) {
            if (i < firstDayOfMonth) {
                ...
                if (isAvailable) {
                    html += '<td class="available-day" onclick="showDay(' + dayCounter + ', ' + month + ', ' + year +
                        ')">' + dayCounter + '</td>';
                } else {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                if (dayCounter <= daysInMonth) {
                    ...
                    console.log(fechasDisponibles.includes(fechaFormateada));
                    if (isAvailable) {
                        html += '<td class="available-day" onclick="showDay(' + dayCounter + ', ' + month + ', ' +
                            year + ')">' + dayCounter + '</td>';
                    } else {
                        html += '<td class="unavailable-day">' + dayCounter + '</td>';
                    }
    ...
    function showDay(day, month, year) {
        ...
        document.getElementById('fecha_seleccionada').value = year + '-' + (month < 10 ? '0' + month : month) + '-' + (
            day < 10 ? '0' + day : day);
